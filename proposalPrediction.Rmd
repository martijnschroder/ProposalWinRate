...
# Remove code from the set as this is redundant given account
train_set <- train_set %>% select(-code)
test_set <- test_set %>% select(-code)
...
accuracy_results
```
One challenge with glm was that the trained model didn't generalise for features in the test set that weren't part of the training set. Other models did generalise, which for predcition of proposal win rate is required. To overcome this challenge, we could ensure that all factors in the features are present in both training and test set, however that would defeat the purpose of this exercise.

To overcome this challenge I quantified the information using WOE. The results reported were generated in this manner.

The learning is that the type of data collected may need to be expanded with features that are agnistic to offers, accounts and practices. This could include level of confidence to win the proposal, match between proposal request and skills and quality of the relationship.
